"""Solve sound-alias names differing from a known alias in their final two bytes."""
import argparse
import os
import sys
...
def main(argv):
    ...
    targets = [asset_id for asset_id, pool in snapshot.read(next(
        path for path in snapshot.snapshots()
        if snapshot.read(path).game.lower() == options.game.lower())).unnamed(known).items()
               if pool == "sound_alias"]
