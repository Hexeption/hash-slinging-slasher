"""18-Hour Autonomous High-Yield Recovery Suite across Black Ops 4 and Cold War.

Configured for unattended overnight execution (~18 hours):
...
- Multi-tier round architecture designed for sustained yields across an 18-hour window:
    Tier 1 (High density):
      * BO4 SAB sound all-boundary (segments 2, 1, 3) with game-correct backslashes
      * Sound-specific backwards final-byte solve (with --no-fold on BO4)
      * All-boundary sound uncarried endings (segments 2, 1, 3 across both games)
      * All-boundary general uncarried endings (segments 2, 3, 1, 4 across both games)
      * Confirmed-only all-boundary cores (sound & general, segment 2)
      * Corpus-mined substitutions, equivalence classes, and indels (top 400, 800)
      * Multi-core tails (length 4)
    Tier 2 (Deep exploration):
      * Deeper segment sweeps (sound segment 4, general segment 5, top 120k)
      * Confirmed-only all-boundary cores (segments 1 and 3)
      * Extended mined axes (top 1600)
      * Full-space tails (length 5, ~43T candidates with backwards peeling)
      * Measured shells (head 6 + tail 6 cross products)
- Immediately triggers derive_closure and community submit.exe after every finding.
"""
...
def post_pass_actions(game: str = None):
    ...
    # Ensure empty_runs ledger does not trigger false futility blocks during derivations
    empty_runs_file = ROOT / "state" / "empty_runs.txt"
    if empty_runs_file.exists():
        try:
            empty_runs_file.write_text("0\n", encoding="utf-8")
        except Exception:
            pass
...
def preflight_sync():
    """Run start.exe to refresh hash tables, survey open PRs, and renew readiness lease."""
...
def job_tails(game: str, length: int = 4):
    ...
    code, _ = run_cmd([sys.executable, str(script), "--length", str(length), "--write-plan", str(plan_file)], timeout=900)
    if code == 0 and plan_file.exists():
        timeout_sec = 10800 if length >= 5 else 7200
        code2, out = run_cmd(
            [
                str(confirm_bin),
                str(plan_file),
...
def job_measured_shells(game: str, head: int = 6, tail: int = 6, top: int = 800):
    ...
    out_prefix = ROOT / "plans" / f"shell_h{head}t{tail}"
    code, _ = run_cmd([sys.executable, str(script), "--head", str(head), "--tail", str(tail), "--top", str(top), "--out", str(out_prefix)], timeout=600)
    if code == 0 and plan_file.exists():
        code2, out = run_cmd(
            [
                str(confirm_bin),
...
# =========================================================================
# MAIN ORCHESTRATION LOOP (18 HOURS)
# =========================================================================

def main():
    parser = argparse.ArgumentParser(description="Overnight autonomous recovery suite across Black Ops 4 and Cold War")
    parser.add_argument("--hours", type=float, default=18.0, help="Duration to run in hours (default: 18.0)")
    args = parser.parse_args()

    max_duration_sec = int(args.hours * 3600)
    log(f"=== STARTING {args.hours:.1f}-HOUR AUTONOMOUS RECOVERY SUITE ===")
    ...
    while time.time() - start_time < max_duration_sec:
        ...
        # -------------------------------------------------------------
        # Tier 1: High-Density Passes (Both Games)
        # -------------------------------------------------------------
        ...
        # 3. All-Boundary Sound Uncarried Endings
        if time.time() - start_time >= max_duration_sec:
            break
        for seg in [2, 1, 3]:
        ...
        # 4. All-Boundary General Uncarried Endings
        if time.time() - start_time >= max_duration_sec:
            break
        for seg in [2, 3, 1, 4]:
        ...
        # 5. Confirmed-Only All-Boundary Cores (Segment 2)
        if time.time() - start_time >= max_duration_sec:
            break
        ...
        # 6. Mined Axes (Top 400, 800, Classes, Indels)
        if time.time() - start_time >= max_duration_sec:
            break
        ...
        # -------------------------------------------------------------
        # Tier 2: Deep Exploration Passes (Alternating in Later Rounds)
        # -------------------------------------------------------------
        if iteration % 2 == 0 or time.time() - start_time > 4 * 3600:
            log(f"--- [TIER 2] Deep Exploration & Extended Bounds: Round {iteration} ---")

            # BO4 SAB sound deeper segments (segment 4)
            if time.time() - start_time < max_duration_sec:
                job_bo4_sound_allboundary(segments=4, top=80000)

            # Deeper all-boundary sound & general (higher tops and deeper segment cuts)
            if time.time() - start_time < max_duration_sec:
                job_allboundary_sound("BLKOPSCW", segments=4, top=80000)
                job_allboundary_sound("BLKOPS04", segments=4, top=80000)

            if time.time() - start_time < max_duration_sec:
                job_allboundary_general("BLKOPSCW", segments=5, top=120000)
                job_allboundary_general("BLKOPS04", segments=5, top=120000)

            # Confirmed-only cores across segment depths 1 and 3
            if time.time() - start_time < max_duration_sec:
                job_allboundary_confirmed_only("BLKOPSCW", sound=True, segments=1, top=60000)
                job_allboundary_confirmed_only("BLKOPS04", sound=True, segments=1, top=60000)
                job_allboundary_confirmed_only("BLKOPSCW", sound=False, segments=3, top=100000)
                job_allboundary_confirmed_only("BLKOPS04", sound=False, segments=3, top=100000)

            # Extended mined axes (top 1600)
            if time.time() - start_time < max_duration_sec:
                job_mined_axes("BLKOPSCW", top=1600)
                job_mined_axes("BLKOPS04", top=1600)

            # Measured shells (head 6 tail 6)
            if time.time() - start_time < max_duration_sec:
                job_measured_shells("BLKOPSCW", head=6, tail=6, top=800)
                job_measured_shells("BLKOPS04", head=6, tail=6, top=800)

            # Full-space tails (Length 5)
            if time.time() - start_time < max_duration_sec:
                job_tails("BLKOPSCW", length=5)
                job_tails("BLKOPS04", length=5)

        iteration += 1

    log(f"=== OVERNIGHT SUITE COMPLETED ({args.hours:.1f} HOURS) ===")
    post_pass_actions()
