"""Probe aliases formed by appending their first underscore token."""
import os, sys
...
for name in known:
    ...
    candidate = name + "_" + first
    if candidate not in known:
        seen.add(candidate)
print(f"{len(known)} known sound aliases; {len(seen)} first-token appends", file=sys.stderr)
