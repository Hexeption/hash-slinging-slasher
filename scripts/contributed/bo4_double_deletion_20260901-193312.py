"""Emit BO4 names formed by deleting two distinct basename characters.

Directory components are preserved; this probes two-character spelling and padding
changes that single-character edits cannot reach.
"""
import argparse
import os
import sys
...
def names(kind):
    vals = list(snapshot.table_names(TABLES[kind]))
    vals += list(snapshot.confirmed_names(CONFIRMED[kind]))
