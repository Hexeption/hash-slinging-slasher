"""Probe sound aliases formed by replacing one underscore with a hyphen."""
import os, sys
    ...
    for i, char in enumerate(name):
        if char == "_":
            ...
            if candidate not in known:
                seen.add(candidate)
print(f"{len(known)} known sound aliases; {len(seen)} underscore-to-hyphen candidates", file=sys.stderr)
